...
for e in range(epochs):
    ...
    real_indices_ = np.intersect1d(np.where(gen_error >0)[0], idx_)
...
for hc in pbar(range(n_sample)):
    try:
        #sample_y = np.random.uniform(s_min, s_max, size = [1,])
        sample_y = np.random.uniform(1.3 * s_min, s_max, size = [1,])
        ...
        accurate = np.where(gen_errors >0)[0]
...
output.to_csv('generated_SMILES_new.csv', index = False)
