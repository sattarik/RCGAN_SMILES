...
class Config:
    ...
    def __init__(self):
        ...
        self.genFilters = [128, 128, 128, 128, 128]
        self.upFilters = [(2, 2), (2, 2), (2, 2), (2,2), (2,2)]
...
def build_combined(z, y,
                   ...
                   regressor_top,
                   discriminator):
    atoms_emb, bonds_emb = encoder(generator([z, y]))
...
#regressor = load_model('./../data/nns_newgen/regressor.h5')
#regressor_top = load_model('./../data/nns_newgen/regressor_top.h5')
...
regressor.save    ('./../data/nns_newgen/regressor.h5')
regressor_top.save('./../data/nns_newgen/regressor_top.h5')
...
for e in range(epochs):
    ...
    for b in range(batches):
        ...
        gen_atoms_embedding, gen_bonds_embedding = encoder.predict(generator.predict([batch_z, batch_y]))
        """
    ...
    for _ in range(reinforce_sample):
        ...
        sample_atoms_embedding, sample_bonds_embedding = encoder.predict(generator.predict([sample_z, sample_y]))
...
generator.save('./../data/nns_newgen/generator.h5')
discriminator.save('./../data/nns_newgen/discriminator.h5')
...
for hc in pbar(range(n_sample)):
    try:
        ...
        sample_atoms_embedding, sample_bonds_embedding = encoder.predict(generator.predict([sample_z, sample_y_]))
